...
import com.arne5.invaderyb.screen.*;
			...
			{   player = new Player(new Vector2(230,15),new Vector2(0,0),Assets.PLAYER.getWidth(),Assets.PLAYER.getHeight(),this,camera);
				...
				//prefs.putInteger("HighScore", 50);
						...
						for(int i=0;i <amount;i++)
							{
								float x = MathUtils.random(0, InvaderYB.WIDTH- Assets.ENEMY.getWidth());
								float y = MathUtils.random(InvaderYB.HEIGHT, InvaderYB.HEIGHT *3);
				...
				if(runLevel2)
					{
						for(int i=0;i <10;i++)
							{
								float x = MathUtils.random(0,InvaderYB.WIDTH-Assets.greenEnemy.getWidth());
								float y = MathUtils.random(InvaderYB.HEIGHT,InvaderYB.HEIGHT*3);
								float speed = MathUtils.random(2,5);
